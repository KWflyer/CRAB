...
from model.layers.attention_layers import SEModule, CBAM, PAB
...
class CSPBlock(nn.Module):
    def __init__(
        ...
        elif self.attention == "PAB":
            self.attention_module = PAB(out_channels)
        else:
            self.attention = None
    ...
    def forward(self, x):
        residual = x
        out = self.block(x)
        if self.attention is not None and self.use_att is not False:
...
class CSPFirstStage(nn.Module):
    def __init__(self, in_channels, out_channels, use_att=True):
...
class CSPStage(nn.Module):
    def __init__(self, in_channels, out_channels, num_blocks, use_att=True):
        ...
        self.blocks_conv = nn.Sequential(
            Convolutional(out_channels, out_channels//2, 1),
            *[
                CSPBlock(out_channels//2, out_channels//2, use_att=use_att)
                for _ in range(num_blocks)
            ],
            Convolutional(out_channels//2, out_channels, 1)
        )
...
class CSPDarknet53(nn.Module):
    def __init__(
        ...
        self.stages = nn.ModuleList(
            [
                CSPFirstStage(stem_channels, feature_channels[0], use_att=u),

                CSPStage(feature_channels[0], feature_channels[1], 2, use_att=u),

                CSPStage(feature_channels[1], feature_channels[2], 8, use_att=u),

                CSPStage(feature_channels[2], feature_channels[3], 8, use_att=u),

                CSPStage(feature_channels[3], feature_channels[4], 4, use_att=False),
            ]
        )
